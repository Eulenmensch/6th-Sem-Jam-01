    ...
    public class HBAORendererFeature : ScriptableRendererFeature
    {
        ...
        private class HBAORenderPass : ScriptableRenderPass
        {
            ...
            private static class ShaderProperties
            {
                ...
                public static string GetDirectionsKeyword(HBAORendererSettings.GeneralSettings settings)
                {
                ...
                public static string GetStepsKeyword(HBAORendererSettings.GeneralSettings settings)
                {
                    switch (settings.quality)
                    {
                        case Quality.Lowest:
                ...
                public static string GetNoiseKeyword(HBAORendererSettings.GeneralSettings settings)
                {
                ...
                public static string GetDeinterleavingKeyword(HBAORendererSettings.GeneralSettings settings)
                {
                ...
                public static string GetDebugKeyword(HBAORendererSettings.GeneralSettings settings)
                {
                ...
                public static string GetMultibounceKeyword(HBAORendererSettings.AOSettings settings)
                {
                    return settings.useMultiBounce ? "MULTIBOUNCE" : "__";
                }

                public static string GetOffscreenSamplesContributionKeyword(HBAORendererSettings.AOSettings settings)
                {
                    return settings.offscreenSamplesContribution > 0 ? "OFFSCREEN_SAMPLES_CONTRIBUTION" : "__";
                }

                public static string GetPerPixelNormalsKeyword(HBAORendererSettings.AOSettings settings)
                {
                ...
                public static string GetBlurRadiusKeyword(HBAORendererSettings.BlurSettings settings)
                {
                ...
                public static string GetVarianceClippingKeyword(HBAORendererSettings.TemporalFilterSettings settings)
                {
                ...
                public static string GetColorBleedingKeyword(HBAORendererSettings.ColorBleedingSettings settings)
                {
            ...
            private bool isHistoryBufferDirty
            {
                get
                {
                    if (m_PreviousTemporalFilterEnabled != settings.temporalFilter.enabled ||
                        m_PreviousResolution != settings.general.resolution || m_PreviousWidth != aoDesc.width || m_PreviousHeight != aoDesc.height ||
                        m_PreviousColorBleedingEnabled != settings.colorBleeding.enabled)
                    {
                        m_PreviousTemporalFilterEnabled = settings.temporalFilter.enabled;
                        m_PreviousResolution = settings.general.resolution;
            ...
            public void Setup(Shader shader, ScriptableRenderer renderer, RenderingData renderingData)
            {
                ...
                renderPassEvent = settings.general.debugMode == DebugMode.Disabled ?
            ...
            public override void Configure(CommandBuffer cmd, RenderTextureDescriptor cameraTextureDescriptor)
            {
                ...
                if (isHistoryBufferDirty && settings.temporalFilter.enabled)
            ...
            public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
            {
                ...
                // AO
                if (settings.general.deinterleaving == Deinterleaving.Disabled)
                {
            ...
            private void FetchRenderParameters(RenderTextureDescriptor cameraTextureDesc)
            {
                ...
                var downsamplingFactor = settings.general.resolution == Resolution.Full ? 1 : settings.general.deinterleaving == Deinterleaving.Disabled ? 2 : 1;
            ...
            private void AllocateHistoryBuffers()
            {
                ...
                if (settings.colorBleeding.enabled)
            ...
            private void CopySource(CommandBuffer cmd)
            {
                ...
                Blit(cmd, source, ShaderProperties.inputTex, material, Pass.Copy);
            }
            ...
            {
                if (settings.blur.type != BlurType.None)
                {
                    ...
                    Blit(cmd, ShaderProperties.hbaoTex, ShaderProperties.tempTex, material, Pass.Blur);
                    cmd.SetGlobalVector(ShaderProperties.blurDeltaUV, new Vector2(0, 1f / sourceDesc.height));
                    Blit(cmd, ShaderProperties.tempTex, ShaderProperties.hbaoTex, material, Pass.Blur);
                    cmd.ReleaseTemporaryRT(ShaderProperties.tempTex);
                }
            }
            ...
            {
                if (settings.temporalFilter.enabled && !renderingInSceneView)
                {
                    if (settings.colorBleeding.enabled)
                    {
                        ...
                        Blit(cmd, aoHistoryBuffer, ShaderProperties.tempTex2, material, Pass.Copy);
                        Blit(cmd, colorBleedingHistoryBuffer, ShaderProperties.tempTex, material, Pass.Copy);
                        cmd.SetRenderTarget(rts, rts[0]);
                        cmd.SetGlobalTexture(ShaderProperties.mainTex, ShaderProperties.tempTex2);
                        cmd.DrawMesh(RenderingUtils.fullscreenMesh, Matrix4x4.identity, material, 0, Pass.Temporal_Filter);
                        cmd.ReleaseTemporaryRT(ShaderProperties.tempTex);
                    ...
                    else
                    {
                        ...
                        cmd.GetTemporaryRT(ShaderProperties.tempTex, aoDesc, FilterMode.Bilinear);
                        Blit(cmd, aoHistoryBuffer, ShaderProperties.tempTex, material, Pass.Copy);
                        Blit(cmd, ShaderProperties.tempTex, aoHistoryBuffer, material, Pass.Temporal_Filter);
                        cmd.ReleaseTemporaryRT(ShaderProperties.tempTex);
            ...
            {
                Blit(cmd, ShaderProperties.inputTex, source, material, settings.general.debugMode == DebugMode.ViewNormals ? Pass.Debug_ViewNormals : Pass.Composite);
            }
            ...
            private void UpdateMaterialProperties()
            {
                ...
                float maxRadInPixels = Mathf.Max(16, settings.ao.maxRadiusPixels * Mathf.Sqrt((sourceWidth * sourceHeight) / (1080.0f * 1920.0f)));
                maxRadInPixels /= (settings.general.deinterleaving == Deinterleaving.x4 ? 4 : 1);

                var targetScale = settings.general.deinterleaving == Deinterleaving.x4 ?
                                      new Vector4(reinterleavedAoDesc.width / (float)sourceWidth, reinterleavedAoDesc.height / (float)sourceHeight, 1.0f / (reinterleavedAoDesc.width / (float)sourceWidth), 1.0f / (reinterleavedAoDesc.height / (float)sourceHeight)) :
                                          settings.general.resolution == Resolution.Half /*&& (settings.ao.perPixelNormals == PerPixelNormals.Reconstruct2Samples || settings.ao.perPixelNormals == PerPixelNormals.Reconstruct4Samples)*/ ?
                ...
                material.SetFloat(ShaderProperties.radius, settings.ao.radius * 0.5f * ((sourceHeight / (settings.general.deinterleaving == Deinterleaving.x4 ? 4 : 1)) / (tanHalfFovY * 2.0f)));
                material.SetFloat(ShaderProperties.maxRadiusPixels, maxRadInPixels);
                material.SetFloat(ShaderProperties.negInvRadius2, -1.0f / (settings.ao.radius * settings.ao.radius));
                material.SetFloat(ShaderProperties.angleBias, settings.ao.bias);
                material.SetFloat(ShaderProperties.aoMultiplier, 2.0f * (1.0f / (1.0f - settings.ao.bias)));
                material.SetFloat(ShaderProperties.intensity, isLinearColorSpace ? settings.ao.intensity : settings.ao.intensity * 0.454545454545455f);
                material.SetFloat(ShaderProperties.multiBounceInfluence, settings.ao.multiBounceInfluence);
                material.SetFloat(ShaderProperties.offscreenSamplesContrib, settings.ao.offscreenSamplesContribution);
                material.SetFloat(ShaderProperties.maxDistance, settings.ao.maxDistance);
                material.SetFloat(ShaderProperties.distanceFalloff, settings.ao.distanceFalloff);
                material.SetColor(ShaderProperties.baseColor, settings.ao.baseColor);
                material.SetFloat(ShaderProperties.blurSharpness, settings.blur.sharpness);
                material.SetFloat(ShaderProperties.colorBleedSaturation, settings.colorBleeding.saturation);
                material.SetFloat(ShaderProperties.colorBleedBrightnessMask, settings.colorBleeding.brightnessMask);
                material.SetVector(ShaderProperties.colorBleedBrightnessMaskRange, AdjustBrightnessMaskToGammaSpace(new Vector2(Mathf.Pow(settings.colorBleeding.brightnessMaskRange.x, 3), Mathf.Pow(settings.colorBleeding.brightnessMaskRange.y, 3))));
                material.SetVector(ShaderProperties.temporalParams, settings.temporalFilter.enabled && !renderingInSceneView ? new Vector2(s_temporalRotations[frameCount % 6] / 360.0f, s_temporalOffsets[frameCount % 4]) : Vector2.zero);
            }
            ...
            private void UpdateShaderKeywords()
            {
                ...
                m_ShaderKeywords[1] = ShaderProperties.GetDirectionsKeyword(settings.general);
                m_ShaderKeywords[2] = ShaderProperties.GetStepsKeyword(settings.general);
                m_ShaderKeywords[3] = ShaderProperties.GetNoiseKeyword(settings.general);
                m_ShaderKeywords[4] = ShaderProperties.GetDeinterleavingKeyword(settings.general);
                m_ShaderKeywords[5] = ShaderProperties.GetDebugKeyword(settings.general);
                m_ShaderKeywords[6] = ShaderProperties.GetMultibounceKeyword(settings.ao);
                m_ShaderKeywords[7] = ShaderProperties.GetOffscreenSamplesContributionKeyword(settings.ao);
                m_ShaderKeywords[8] = ShaderProperties.GetPerPixelNormalsKeyword(settings.ao);
                m_ShaderKeywords[9] = ShaderProperties.GetBlurRadiusKeyword(settings.blur);
                m_ShaderKeywords[10] = ShaderProperties.GetVarianceClippingKeyword(settings.temporalFilter);
                m_ShaderKeywords[11] = ShaderProperties.GetColorBleedingKeyword(settings.colorBleeding);
            ...
            {
                if (settings.general.deinterleaving != Deinterleaving.Disabled && SystemInfo.supportedRenderTargetCount < 4)
                    settings.SetDeinterleaving(Deinterleaving.Disabled);

                if (settings.temporalFilter.enabled && !motionVectorsSupported)
                    settings.EnableTemporalFilter(false);

                if (settings.colorBleeding.enabled && settings.temporalFilter.enabled && SystemInfo.supportedRenderTargetCount < 2)
                    settings.EnableTemporalFilter(false);

                // Noise texture
                if (noiseTex == null || m_PreviousNoiseType != settings.general.noiseType)
                {
                    CoreUtils.Destroy(noiseTex);

                    CreateNoiseTexture();

                    m_PreviousNoiseType = settings.general.noiseType;
                }
            }
            ...
            private void CreateNoiseTexture()
            {
                ...
                for (int x = 0; x < 4; ++x)
                {
                    for (int y = 0; y < 4; ++y)
                    {
                        float r1 = settings.general.noiseType != NoiseType.Dither ? 0.25f * (0.0625f * ((x + y & 3) << 2) + (x & 3)) : MersenneTwister.Numbers[z++];
                        float r2 = settings.general.noiseType != NoiseType.Dither ? 0.25f * ((y - x) & 3) : MersenneTwister.Numbers[z++];
        ...
        public override void AddRenderPasses(ScriptableRenderer renderer, ref RenderingData renderingData)
        {
            ...
            {
#if UNITY_EDITOR
                var settingsAssetPath = AssetDatabase.GUIDToAssetPath("68dde9df85a05a54fbc92e3b6f3a8c42");
                if (settingsAssetPath != null && settingsAssetPath.Length > 0)
                {
                    settings = AssetDatabase.LoadAssetAtPath(settingsAssetPath, typeof(HBAORendererSettings)) as HBAORendererSettings;
                }
            ...
            if (!settings.general.enable) return;
